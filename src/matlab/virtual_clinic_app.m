...
% VIRTUAL_CLINIC_APP Interactive UI for eDRis
    ...
    uilabel(tgl, 'Text', 'eDRis Clinical Dashboard', 'FontSize', 24, 'FontWeight', 'bold');
    uilabel(tgl, 'Text', 'Automated Rural DR Screening System (MathWorks SIH 26038)', 'FontSize', 12, 'FontColor', [0.4 0.4 0.4]);
    ...
    app.pnlPatient = uipanel(app.dashGrid, 'Title', 'Patient Demographics', 'BackgroundColor', [1 1 1], 'FontWeight', 'bold');
    ...
    app.pnlQC = uipanel(app.dashGrid, 'Title', 'Phase 1: Image Quality Assessment', 'BackgroundColor', [1 1 1], 'FontWeight', 'bold');
    app.pnlQC.Layout.Row = 2; app.pnlQC.Layout.Column = 2;
    app.pnlQC.Visible = 'off';
    
    qgl = uigridlayout(app.pnlQC, [3, 1]);
    app.lblBlur = uilabel(qgl, 'Text', '');
    app.lblIllum = uilabel(qgl, 'Text', '');
    app.lblStatus = uilabel(qgl, 'Text', 'Status: PASSED & ENHANCED (Adaptive CLAHE)', 'FontWeight', 'bold', 'FontColor', [0.1 0.6 0.1]);

    % 4. Confidence Gauge
    app.pnlGauge = uipanel(app.dashGrid, 'Title', 'Phase 3: Diagnostic Confidence', 'BackgroundColor', [1 1 1], 'FontWeight', 'bold');
    ...
    app.lblConf = uilabel(ggl, 'Text', '', 'FontSize', 28, 'FontWeight', 'bold', 'FontColor', [0.2 0.2 0.6], 'HorizontalAlignment', 'center');

    % 5. Image Axes
    app.ax1 = uiaxes(app.dashGrid); app.ax1.Layout.Row = 3; app.ax1.Layout.Column = 1; app.ax1.Visible = 'off';
    app.ax2 = uiaxes(app.dashGrid); app.ax2.Layout.Row = 3; app.ax2.Layout.Column = 2; app.ax2.Visible = 'off';
    app.ax3 = uiaxes(app.dashGrid); app.ax3.Layout.Row = 3; app.ax3.Layout.Column = 3; app.ax3.Visible = 'off';
    ...
    function uploadImage()
        ...
        try
            ...
            imshow(imread(img_path), 'Parent', app.ax1); title(app.ax1, 'Raw Fundus Capture', 'FontSize', 12);
            imshow(clean_img, 'Parent', app.ax2); title(app.ax2, 'Gatekeeper Standardized Image', 'FontSize', 12);
            ...
            hold(app.ax3, 'off');
            title(app.ax3, 'Phase 4: Explainability (Grad-CAM)', 'FontSize', 12);
